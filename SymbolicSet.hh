...
private:
  ...
  template<class preStateType_, class inputType_, class postStateType_> friend class SymbolicModelUnbalanced;
...
public:
  /* constructor: SymbolicSet :: default, added by MK 01.09/.2016
   */
  SymbolicSet(){
  }
   ...
   */
	SymbolicSet(Cudd &ddmgr, const size_t dim, const double* lb, const double* ub, const double* eta, const double* z=NULL, bool GenerateNewBddVars=true) {
    ...
    for (size_t i=0; i<dim; i++) {
      ...
      /* ******************************************** */
      /* MODIFIED BY MK @ 31.08.2016 **************** */
      /* ******************************************** */
      if(GenerateNewBddVars){
  ...
  /*
  void PrintBDD(std::string NameIt, BDD& bddObj){
  ...
  */
  ...
  std::vector<std::vector<double>> setValuedMap(std::vector<double> x, std::vector<size_t> ind) {
    ...
    // MK TEST:: BDD PRINT INFO
    //std::cout << "-------------------------------------------------------" << std::endl;
    //PrintBDD("cube",cube);
    //std::cout << "-------------------------------------------------------" << std::endl;
    //PrintBDD("symbolicSet_",symbolicSet_);
    //std::cout << "-------------------------------------------------------" << std::endl;
    // MK TEST:: END
